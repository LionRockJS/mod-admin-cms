...
export default class ControllerAdminPage extends ControllerAdmin {
    ...
    isPostOriginalDiff(postOriginal, original, action) {
        ...
        if (!postOriginal.blocks)
            return false; //no blocks in postOriginal, no diff
    ...
    async saveVersion(postOriginal, original, id, slug, action) {
        if (!this.isPostOriginalDiff(postOriginal, original, action))
            return;
    ...
    async action_update() {
        ...
        if (action === 'publish_weights')
            await this.publish_weights();
    ...
    async updateLiveSchedule(instance) {
        ...
        if (!livePage)
            return;
        if (livePage.start === instance.start && livePage.end === instance.end)
            return;
            ...
            Object.keys(block.tokens).forEach(token => {
                if (Array.isArray(block.tokens[token])) {
                    block.tokens[token].forEach((it, j) => {
                        if (typeof it !== 'object')
                            return; //do nothing for block fields;
                        // block items
                        it._weight = parseInt(it._weight || "0");
                        it._key = j;
    ...
    async action_edit() {
        ...
        page.page_tags = page.page_tags || [];
        await Promise.all(page.page_tags.map(async (page_tag) => {
            try {
    ...
    async action_restore() {
        ...
        if (!trashPage)
            throw new Error(`Page ${id} not found in trash`);
    ...
    async action_delete() {
        if (this.state.get(ControllerMixinORMDelete.DELETED)) {
            ...
            this.state.set(ControllerMixinCRUDRedirect.REDIRECT, this.state.get(ControllerMixinCRUDRedirect.REDIRECT) || `/admin/pages/list/${page.page_type}`);
        }
    }
    ...
    async block_add(page, blockName) {
        ...
        if (!blueprint)
            throw new Error(`Block ${blockName} not defined in config`);
