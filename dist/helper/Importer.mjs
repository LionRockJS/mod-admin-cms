...
export default class HelperImporter {
    ...
    static getUpdateSummary(safeFields, conflictFields = null, tagComparison = null) {
        ...
        if (conflictFields && conflictFields.attributes && conflictFields.attributes.length > 0) {
            const conflictAttributeFields = conflictFields.attributes.map(field => `<strong>${field.excelField}</strong>: "${field.databaseValue}" → "${field.excelValue ?? ""}"`);
        ...
        if (conflictFields && conflictFields.values) {
            for (const language in conflictFields.values) {
                ...
                if (languageFields && languageFields.length > 0) {
                    const conflictValueFields = languageFields.map(field => `<strong>${field.excelField}</strong>: "${field.databaseValue}" → "${field.excelValue ?? ""}"`);
        ...
        if (tagComparison && tagComparison.safeToAppend && tagComparison.safeToAppend.length > 0) {
            ...
            tagComparison.safeToAppend.forEach(tag => {
                if (!tagsByType[tag.tagTypeName])
                    tagsByType[tag.tagTypeName] = [];
    ...
    static async validateFile(file, database, uniqueDigestHandler = (item, digest) => { }, findExistingRecord = async (item, database) => { }, configImporter = { attributes: [], values: {}, items: [], tags: [] }) {
    ...
    static assignAttributes(original, item, keys, shouldTrim = true) {
        keys.forEach(key => {
            if (Array.isArray(key)) {
                ...
                if (!(key[0] in item))
                    return;
                if (!original.attributes[key[1]] && !item[key[0]])
                    return;
                ...
                if (!(key in item))
                    return;
                if (!original.attributes[key] && !item[key])
                    return;
        ...
        configs.forEach(config => {
            ...
            config[netItemType].forEach(it => {
                if (typeof it === 'string') {
                    ...
                    if (!(sourceKey in item))
                        return;
                    if (!item[sourceKey])
                        return; // no data in source cell, skip
            ...
            if (existItem.length > 0) {
                ...
                existItem.forEach(it => {
                    if (this.compareItems(newItem, it))
                        changed = false;
                });
            }
            if (!changed)
    ...
    static compareItems(obj1, obj2) {
        ...
        if (Object.keys(obj1.attributes).length !== Object.keys(obj2.attributes).length)
            return false;
        ...
        if (!obj1.values[defaultLanguage] || !obj2.values[defaultLanguage])
            return true;
        if (Object.keys(obj1.values[defaultLanguage]).length !== Object.keys(obj2.values[defaultLanguage]).length)
            return false;
    ...
    static async importFile(file, database, Model, configImporter = { attributes: [], values: {}, items: [] }, validateFile = this.validateFile, findExistingRecord = async (item, database) => { }, handler = async (original, item, autoId) => { }, action = HelperImporter.ACTION_NEW_AND_APPEND) {
        ...
        if (action === this.ACTION_NEW_ONLY || action === this.ACTION_NEW_AND_APPEND) {
            for (const item of added) {
                ...
                if (configImporter.attributes)
                    this.assignAttributes(original, item, configImporter.attributes);
                if (configImporter.values)
                    this.assignValues(original, item, configImporter.values);
                if (configImporter.items)
                    this.assignItems(original, item, configImporter.items);
        ...
        if (action === this.ACTION_APPEND_ONLY || action === this.ACTION_NEW_AND_APPEND) {
            for (const item of duplicated) {
                ...
                if (!item.__safeFields && !hasSafeTags)
                    continue;
                const instance = item.__instance;
                const original = HelperPageText.getOriginal(instance);
                                ...
                                fields.forEach(field => {
                                    if (typeof field.dbField === 'string') {
                                        ...
                                        if (!targetItem.values)
                                            targetItem.values = {};
                                        if (!targetItem.values[language])
                                            targetItem.values[language] = {};
        ...
        if (action === this.ACTION_OVERWRITE || action === this.ACTION_FORCE_NEW) {
            for (const item of duplicated) {
                if (!item.__safeFields && !item.__conflictFields)
                    continue; // Skip if no safe fields to import
                const instance = item.__instance;
                const original = HelperPageText.getOriginal(instance);
                if (configImporter.attributes)
                    this.assignAttributes(original, item, configImporter.attributes);
